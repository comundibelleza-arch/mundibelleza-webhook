// api/agente-ventas-blackhair-landing.js
//
// Webhook de Salesbot (widget_request) para el agente de ventas de la
// Tintura Líquida Negra / BlackHair Shampoo — versión "landing" (embudo
// directo, sin la pre-calificación de canas/objetivo): el saludo ya
// muestra los 3 combos con precio y pregunta directo "¿Cuál te interesa?".
// Se llama "landing" porque es la versión pensada para tráfico que ya
// llega con intención de compra (por ejemplo desde una landing page/anuncio
// con oferta explícita), a diferencia de agente-ventas-blackhair.js (el
// archivo "principal"), que además incluye el embudo de pre-calificación
// para tráfico frío (canas → objetivo → invitación a ver ofertas → precio).
//
// Sigue el mismo patrón que extraer-datos.js y enviar_catalogo.js: lee el
// mensaje y el lead_id del body/token, decide qué responder, guarda lo que
// aprenda en los campos del lead, y le avisa a Kommo (return_url) que
// continúe el bot.
//
...
const FAQ_SIEMPRE = [
  ...
  {
    patron:
      /(alergia|al[eé]rgic|\bppd\b|reacci[oó]n\s*(al[eé]rgica)?|irritaci[oó]n|piel\s*sensible|prueba\s*de\s*(alergia|sensibilidad)|henna)/,
    respuesta: () =>
...
async function leerEstadoDelLead(leadId) {
  const resp = await fetch(`https://${KOMMO_SUBDOMAIN}.kommo.com/api/v4/leads/${leadId}`, {
...
async function llamarLLM(estado, mensajeNuevo) {
  ...
  const response = await fetch("https://api.anthropic.com/v1/messages", {
    ...
    body: JSON.stringify({
      ...
      messages: [{ role: "user", content: `${resumenEstado}\n\nMensaje del cliente: "${mensajeNuevo}"` }],
    }),
  });
...
async function agregarNotaDePedidoCerrado(leadId, estado) {
  ...
  const resp = await fetch(`https://${KOMMO_SUBDOMAIN}.kommo.com/api/v4/leads/${leadId}/notes`, {
    method: "POST",
    headers: {
      Authorization: `Bearer ${KOMMO_TOKEN}`,
      "Content-Type": "application/json",
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    let leadId =
      (req.body && (req.body.lead_id || (req.body.additional_data && req.body.additional_data.id))) || null;
